# """
# category_ingest_service.py (async job style)

# What changed (Option 2):
# - POST /ingest returns immediately with 202 + job_id (so client gets "received/processing")
# - Background task does the heavy work (LLM + script generation + optional mongosh execute)
# - GET /ingest/status/{job_id} lets you track: received -> processing -> success/error
# - Terminal logs show: received/queued, processing started, completed/failed

# Notes (local dev):
# - JOBS is in-memory: resets on server restart/reload.
# - If you run multiple workers, each worker has its own JOBS dict (status may “miss” jobs).
#   For production, store JOBS in Redis/Postgres.
...
# def _parse_payload(request: Request, payload_raw: Any, content_type: str) -> Dict[str, Any]:
#     """
#     Centralize payload parsing so both JSON + multipart behave the same.
#     Returns a dict payload.
#     """
#     payload: Dict[str, Any] = {}

#     # JSON body already parsed by caller
...
#         form = payload_raw  # this is a starlette form object
#         payload = {k: _json_maybe(form.get(k)) for k in form.keys()}

#         upload = form.get("file") or form.get("csv")
#         if upload is not None:
#             # UploadFile
#             # NOTE: reading bytes here still happens before we return 202
#             file_bytes = upload.file.read()
#             payload["categories"] = _rows_from_csv_bytes(file_bytes)

#         # some frontends send a JSON field called "payload"
...
# @app.post("/ingest")
# async def ingest(request: Request, background_tasks: BackgroundTasks, x_api_key: Optional[str] = Header(default=None)):
...
#         payload = _parse_payload(request, raw, ct)
#     elif "multipart/form-data" in ct:
#         form = await request.form()
#         payload = _parse_payload(request, form, ct)
#     else:
#         raise HTTPException(status_code=415, detail=f"Unsupported Content-Type: {ct}")

#     # Ensure request_id exists; reuse it as job_id (also helps downstream filename suffix)
#     job_id = str(payload.get("request_id") or uuid.uuid4().hex)
#     payload["request_id"] = job_id

#     # Normalize runtime options (same behavior as before)
#     opts: Dict[str, Any] = {
...
#     # Job metadata for quick “received/queued” confirmation
...
#     # Run heavy work after returning response
#     background_tasks.add_task(_run_ingest_job, job_id, payload, opts)
